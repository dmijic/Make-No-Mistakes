...
description: "Apply the Make No Mistakes (MNM) protocol in Claude Code: recover durable project state, inspect the repository, implement bounded changes, verify with evidence, and escalate rather than silently redesign consequential behavior."
---

# Software Project Workflow (Claude Code)

Claude Code is an **execution-oriented** MNM runtime by default: it operates directly on a repository. This is a default emphasis, not a restriction — see `adapters/README.md`.
...
- `guidance/` — optional practical guidance (Git hygiene, external capabilities, workflow composition)
- `templates/` — optional artifacts (`handoff.md`, `review.md`)
- `capabilities/` — reusable domain expertise (currently: application security, GenAI/LLM security)
...
The repository, its canonical documentation and its Git history are persistent project memory; this chat session is a temporary working environment (`core/STATE.md`). Persist consequential state (decisions, invariants, verification evidence) where it belongs in the project — don't rely on conversation continuity to carry it forward. Use `templates/handoff.md` when a bounded objective genuinely needs to cross a session boundary; a SIMPLE task usually doesn't.
